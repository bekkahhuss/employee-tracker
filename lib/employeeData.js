    ...
    const sql =`SELECT department.* FROM department `;
        db.query(sql, (err, rows) => {
            for (let i = 0; i < rows.length; i++) {
                // dept_arr.push("Department Name: " + rows[i].name + ", ID: " + rows[i].id);
...
employeeData.prototype.addEmployee = function() {
    ...
    .then((response) => {
        ...
        const sql = `INSERT INTO employee (id, first_name, last_name, role_id, department_id, manager_id) VALUES (?)`;
        var value = [response.id, response.first_name, response.last_name, response.role_id, response.department_id, response.manager_id]
        db.query(sql, [value], (err, rows) => {
            if (err) throw err;
...
employeeData.prototype.updateEmployeeRole = function (){
    ...
    const sql = `SELECT employee.* FROM employee`;
        ...
        function updateEmployee(name) {
            ...
            .then((response) => {
                const sql = `UPDATE employee set role_id = '${response.role}', department_id = WHERE first_name = '${name}'`;
                db.query(sql, (err, rows) => {
                    if (err) throw err;
